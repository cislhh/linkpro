...
async function getUserByUsername(username: string): Promise<UserData | null> {
  ...
  // Get all user's links (will be filtered by module selection)
  ...
  const linksModules = await prisma.pageModule.findMany({
    where: {
      userId: userData.id,
      type: "links",
    },
  ...
  linksModules.forEach((module) => {
    try {
      const data = module.data as unknown;
      if (data && typeof data === "object" && "linkIds" in data) {
        const moduleLinkIds = (data as { linkIds: string[] }).linkIds || [];
        selectedLinkIds.push(...moduleLinkIds);
      }
    } catch (e) {
      console.error("Failed to parse links module data:", e);
    }
  });
  ...
  const links =
    selectedLinkIds.length > 0
      ? allLinks.filter((link) => selectedLinkIds.includes(link.id))
      : allLinks;
